...
# Edited by David Goodall, david@mysysadmin.co.uk
#
...
# Change to reflect YOUR EIPs
ALLOCATION_ID_1A="eipalloc-be18ded7"
ALLOCATION_ID_1B="eipalloc-b518dedc"
...
# Set correct EIP for availability zone
if [ "$AVAILABILITY_ZONE" = "eu-central-1a" ]; then
  ALLOCATION_ID=$ALLOCATION_ID_1A
elif [ "$AVAILABILITY_ZONE" = "eu-central-1b" ]; then
  ALLOCATION_ID=$ALLOCATION_ID_1B
else
  die "Availability Zone not valid."
fi

# Associate EIP
${awscmd} ec2 associate-address --instance-id $INSTANCE_ID --allocation-id $ALLOCATION_ID --allow-reassociation
